...
begin
  Writeln('--------------------------------------------------------------');
  Writeln('Ingrese "fin" para terminar de leer empleados');
...
begin
  direccion_inmutable:= 'D:\Codigo\Fundamentos-De-Organizacion-De-Datos-FOD\Practicas-Resueltas\1-Archivos\tp3_bajas_archivos_secuenciales\eje1\archivo\';
  Writeln('Ingrese el nombre que va llevar el archivo');
  readln(nombre_fisico);
  direccion_mutable:= direccion_inmutable + nombre_fisico;
  Assign(archivo_logico, direccion_mutable);
...
//disculpa Gonza
function buscar_empleado(var archivo_logico: archivo; dni_buscado: integer): integer;
var
  e: empleado;
  encontre: boolean;
  pos: integer;
begin
  encontre:= false;
  reset (archivo_logico);
  while (not eof (archivo_logico) and (not encontre)) do
    begin
      read(archivo_logico, e);
      if (e.dni = dni_buscado) then
        encontre:= true;
    end;
  if(encontre)then
    begin
      ...
      // close(archivo_logico);
      Writeln('Posicion del empleado encontrado: ', pos);
      buscar_empleado := pos;
    end
  else
    //  close(archivo_logico);
...
procedure opcion_g(var archivo_logico: archivo);
var
  dni_buscado: Integer;
  pos_buscado: integer; 
  e: empleado;
begin
  ...
  readln(dni_buscado);
  pos_buscado:= buscar_empleado(archivo_logico,dni_buscado);
  // close(archivo_logico);
  Writeln('Posicion del empleado encontrado2222: ', pos_buscado);
  // reset(archivo_logico);
  if(pos_buscado <> -1)then
    begin
      Seek(archivo_logico, FilePos(archivo_logico) - 1);
      read(archivo_logico, e);
      Seek(archivo_logico, pos_buscado);
      write(archivo_logico, e); //lo piso con el ultimo empleado
      Seek(archivo_logico, FilePos(archivo_logico) - 1);
      truncate(archivo_logico); //"borro el ultimo empleado, decremento mi diml"
      Writeln('Empleado eliminado con exito');
    end
  else
    Writeln('No se encontro el empleado con el dni ingresado');
  close(archivo_logico);
end;
...
begin
  ...
  begin
    ...
    Writeln('Integre "G" para dar de BAJA a un empleado');   //<========================================================= Nueva funcionalidad
    Writeln('Ingrese "H" para salir');
    readln(opcion); // Tenes q leer la opción nuevamente después de mostrar las opciones al usuario
